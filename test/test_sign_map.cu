...
void runCUDA( int *quant_inds, char* boundary, char* sign_map, size_t input_size,  uint width, uint height, uint depth)
{
    ...
    cudaMalloc(&d_sign_map, input_size * sizeof(char)); 
    // copy data to device
    ...
    cudaMemcpy(sign_map, d_sign_map, input_size * sizeof(char), cudaMemcpyDeviceToHost); 
...
int main(int argc, char** argv)
{
    ...
    std::vector<char> sign_map(input_size, 0); 
    ...
    runCUDA( quant_inds.get(), boundary.data(), sign_map.data(), input_size,  dims[2], dims[1], dims[0]);
    ...
    writefile<char>("sign_map.uint8", sign_map.data(), input_size);
    writefile<char>("boundary.uint8", boundary.data(), input_size);
    writefile<int>("quant_inds.int32", quant_inds.get(), input_size); 
